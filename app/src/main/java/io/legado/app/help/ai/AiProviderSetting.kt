...
data class AiProviderSetting(
    ...
    // ---- 媒体生成（图片 / 视频）----
    // 留空时按 type 推断，见 AiMediaProtocol.inferFrom
    @SerializedName(value = "mediaProtocol", alternate = ["y"])
    val mediaProtocol: String = "",
    @SerializedName(value = "supportsImageGeneration", alternate = ["ad"])
    val supportsImageGeneration: Boolean = true,
    @SerializedName(value = "supportsVideoGeneration", alternate = ["ae"])
    val supportsVideoGeneration: Boolean = true,
    /** 留空则取所选协议的默认值 */
    @SerializedName(value = "imageGenerationsPath", alternate = ["z"])
    val imageGenerationsPath: String = "",
    @SerializedName(value = "imageEditsPath", alternate = ["aa"])
    val imageEditsPath: String = "/images/edits",
    @SerializedName(value = "videoGenerationsPath", alternate = ["ab"])
    val videoGenerationsPath: String = "",
    /** 支持 {id} 占位符 */
    @SerializedName(value = "videoTaskPath", alternate = ["ac"])
    val videoTaskPath: String = "",
    /** 自定义协议族的响应字段映射，留空走宽容解析 */
    @SerializedName(value = "mediaTaskIdPath", alternate = ["af"])
    val mediaTaskIdPath: String = "",
    @SerializedName(value = "mediaStatusPath", alternate = ["ag"])
    val mediaStatusPath: String = "",
    @SerializedName(value = "mediaResultPath", alternate = ["ah"])
    val mediaResultPath: String = "",
    @SerializedName(value = "mediaErrorPath", alternate = ["ai"])
    val mediaErrorPath: String = "",
    /** 图片 / 视频模型，与 chat 用的 [model] 分开保存 */
    @SerializedName(value = "imageModel", alternate = ["aj"])
    val imageModel: String = "",
    @SerializedName(value = "videoModel", alternate = ["ak"])
    val videoModel: String = ""
) {

    val resolvedMediaProtocol: AiMediaProtocol
        get() = if (mediaProtocol.isBlank()) {
            AiMediaProtocol.inferFrom(type)
        } else {
            AiMediaProtocol.from(mediaProtocol)
        }

    fun modelFor(kind: AiMediaKind): String {
        return when (kind) {
            AiMediaKind.IMAGE -> imageModel.ifBlank { model }
            AiMediaKind.VIDEO -> videoModel.ifBlank { model }
            // 配音走设备原生 TTS，不占用任何 Provider 的模型位
            AiMediaKind.AUDIO -> ""
        }
    }

    fun supportsKind(kind: AiMediaKind): Boolean {
        return when (kind) {
            AiMediaKind.IMAGE -> supportsImageGeneration
            AiMediaKind.VIDEO -> supportsVideoGeneration
            // 音频由系统 TTS 提供，与 Provider 的图片/视频能力无关
            AiMediaKind.AUDIO -> false
        }
    }
}
