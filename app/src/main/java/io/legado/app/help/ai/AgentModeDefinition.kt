...
object AgentModeRegistry {
    ...
    const val AI_COMIC_ID = "ai_comic"
    const val AI_COMIC_REVISION = "1"
    const val AI_DRAMA_ID = "ai_drama"
    const val AI_DRAMA_REVISION = "1"
    ...
    val aiComic = AgentModeDefinition(
        id = AI_COMIC_ID,
        revision = AI_COMIC_REVISION,
        kind = AgentModeKind.SYSTEM,
        allowsUserSkills = false,
        allowsManualMcpCapabilities = false,
        systemWorkflowId = AiSkillRegistry.SKILL_AI_COMIC,
        systemSkillIds = listOf(AiSkillRegistry.SKILL_AI_COMIC),
        fixedMcpCapabilityIds = listOf(
            "ai_media.generate_image",
            "ai_media.list_project",
            "ai_media.create_project",
            "ai_media.compose"
        ),
        runtimeHooks = emptyList()
    )

    val aiDrama = AgentModeDefinition(
        id = AI_DRAMA_ID,
        revision = AI_DRAMA_REVISION,
        kind = AgentModeKind.SYSTEM,
        allowsUserSkills = false,
        allowsManualMcpCapabilities = false,
        systemWorkflowId = AiSkillRegistry.SKILL_AI_DRAMA,
        systemSkillIds = listOf(AiSkillRegistry.SKILL_AI_DRAMA),
        fixedMcpCapabilityIds = listOf(
            "ai_media.generate_image",
            "ai_media.generate_video",
            "ai_media.list_project",
            "ai_media.create_project",
            "ai_media.generate_audio",
            "ai_media.compose"
        ),
        runtimeHooks = emptyList()
    )
    ...
    private val definitions = listOf(
        ...
        aiComic,
        aiDrama
    ).associateBy(AgentModeDefinition::id)
