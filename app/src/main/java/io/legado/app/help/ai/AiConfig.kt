...
object AiConfig {
    ...
    const val DEFAULT_MEDIA_TIMEOUT_SECONDS = 900
    const val MIN_MEDIA_TIMEOUT_SECONDS = 60
    const val MAX_MEDIA_TIMEOUT_SECONDS = 3600

    var mediaTimeoutSeconds: Int
        get() = appCtx.getPrefInt(
            PreferKey.aiMediaTimeoutSeconds,
            DEFAULT_MEDIA_TIMEOUT_SECONDS
        ).coerceIn(MIN_MEDIA_TIMEOUT_SECONDS, MAX_MEDIA_TIMEOUT_SECONDS)
        set(value) {
            appCtx.putPrefInt(
                PreferKey.aiMediaTimeoutSeconds,
                value.coerceIn(MIN_MEDIA_TIMEOUT_SECONDS, MAX_MEDIA_TIMEOUT_SECONDS)
            )
        }

    var mediaImageProviderId: String
        get() = appCtx.getPrefString(PreferKey.aiMediaImageProviderId).orEmpty()
        set(value) {
            appCtx.putPrefString(PreferKey.aiMediaImageProviderId, value.trim())
        }

    var mediaImageModelId: String
        get() = appCtx.getPrefString(PreferKey.aiMediaImageModelId).orEmpty()
        set(value) {
            appCtx.putPrefString(PreferKey.aiMediaImageModelId, value.trim())
        }

    var mediaVideoProviderId: String
        get() = appCtx.getPrefString(PreferKey.aiMediaVideoProviderId).orEmpty()
        set(value) {
            appCtx.putPrefString(PreferKey.aiMediaVideoProviderId, value.trim())
        }

    var mediaVideoModelId: String
        get() = appCtx.getPrefString(PreferKey.aiMediaVideoModelId).orEmpty()
        set(value) {
            appCtx.putPrefString(PreferKey.aiMediaVideoModelId, value.trim())
        }

    fun requireMediaModel(kind: AiMediaKind): AiModelSelection {
        val providerId = if (kind == AiMediaKind.IMAGE) mediaImageProviderId else mediaVideoProviderId
        val modelId = if (kind == AiMediaKind.IMAGE) mediaImageModelId else mediaVideoModelId
        check(providerId.isNotBlank() && modelId.isNotBlank()) {
            if (kind == AiMediaKind.IMAGE) {
                "请先在 AI 设置 > AI 创作 中选择图片生成模型"
            } else {
                "请先在 AI 设置 > AI 创作 中选择视频生成模型"
            }
        }
        return AiModelSelection(providerId, modelId)
    }

    fun saveMediaModel(kind: AiMediaKind, providerId: String, modelId: String) {
        if (kind == AiMediaKind.IMAGE) {
            mediaImageProviderId = providerId
            mediaImageModelId = modelId
        } else {
            mediaVideoProviderId = providerId
            mediaVideoModelId = modelId
        }
    }
