...
object McpInternalToolCatalog {
    ...
    val modules: List<McpInternalToolModule> = listOf(
        ...
        module(
            ...
            title = "AI 媒体生成",
            writeCapability(
                id = "ai_media.generate_image",
                title = "AI 图片生成",
                description = "根据提示词生成图片，支持尺寸、比例、质量、风格与参考图（角色垫图）",
                "ai_media_generate_image"
            ),
            writeCapability(
                id = "ai_media.generate_video",
                title = "AI 视频生成",
                description = "根据提示词生成视频，支持时长、比例、首帧/尾帧衔接（漫剧分镜）",
                "ai_media_generate_video"
            ),
            capability(
                id = "ai_media.list_project",
                title = "AI 媒体项目查询",
                description = "按 project_id 回读分镜产物的本地路径与序号，长链生成时防止丢失垫图/首尾帧上下文",
                "ai_media_list_project"
            ),
            writeCapability(
                id = "ai_media.create_project",
                title = "AI 媒体项目创建",
                description = "创建/确保一个命名项目（漫画/漫剧开工第一步），之后分镜产物归到该 project_id",
                "ai_media_create_project"
            ),
            writeCapability(
                id = "ai_media.generate_audio",
                title = "AI 配音生成",
                description = "用设备离线 TTS 把文本合成语音（wav），用于漫剧配音，零成本不占图片/视频 Provider",
                "ai_media_generate_audio"
            ),
            writeCapability(
                id = "ai_media.compose",
                title = "AI 成片拼装",
                description = "format=comic 竖排拼长图，format=drama 用 MediaMuxer 拼接视频；片段编码不一致时回退 storyboard 清单",
                "ai_media_compose"
            )
        )
    )
