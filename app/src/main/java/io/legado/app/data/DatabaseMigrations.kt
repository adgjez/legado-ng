...
object DatabaseMigrations {
     ...
     * 避免 AutoMigration(117,118) 需要 117.json schema（沙箱无法生成），用与 90–113 一致的手动迁移。
     */
    private val migration_117_118 = object : Migration(117, 118) {
        override fun migrate(db: SupportSQLiteDatabase) {
            db.execSQL(
                """CREATE TABLE IF NOT EXISTS `aiMediaProjects` (
                    `id` TEXT NOT NULL,
                    `name` TEXT NOT NULL DEFAULT '',
            ...
            db.execSQL("CREATE INDEX IF NOT EXISTS `index_aiMediaProjects_createdAt` ON `aiMediaProjects` (`createdAt`)")
            db.execSQL("CREATE INDEX IF NOT EXISTS `index_aiMediaProjects_kind` ON `aiMediaProjects` (`kind`)")
        }
    }
